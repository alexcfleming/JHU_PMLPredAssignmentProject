...
dim(crossval4)
dim(testing3)
#we also remove the problem.id column from the final testing set
testing4 = testing3[,-59]
dim(testing4)
```
...
We see with a gradient boosting with trees an accuracy of .9995 on the cross validation set, which seems like a pretty good start. Let's try a tree based model with preprocessing.
...
```{r model3rf}
set.seed(123)
mod3 = randomForest(classe~., data=training4, importance=TRUE, proximity=TRUE)
mod3
...
#We take the predictions and generate the results file for submission
